    ...
    attributes: [
        {
            ...
            pinned: true,          // shown inline on item rows
...
| `renderBody(bodyEl, data)` | Builds the item table with pinned attribute columns; wires up sort headers, expand buttons, drag handlers, and Ctrl+Click name editing |
| `overflowMenuItems(moduleEl, data)` | Returns Add Item and Settings entries |
...
## Interactions
- **Item rows** display: drag handle, item name, pinned attribute values (as icons/badges/text), and an expand button on the far right.
- **Ctrl+Click item name** — opens the shared edit popover to rename the item inline.
...
- **Add Item** — via overflow menu; appends a new item and opens its inspect modal.
...
- **Manage Attributes** — accessible via the overflow menu Settings entry; opens the attribute management panel where the user can add/remove/reorder attributes and launch the Attribute Wizard. Column reorder uses SortableJS drag handles within the manage panel.
...
## Adding Attributes
By default, a new list module has no attributes — items are name-only. The user adds attributes via the **Manage Attributes** panel (accessible from the overflow menu's Settings entry). When an attribute is added, every existing item gains that attribute with the configured default value.
...
## Attribute Pinning
Each attribute has a `pinned` flag. Pinned attributes are displayed inline on item rows, giving the user control over which 2–3 key values are visible at a glance. Pinning is toggled in the **Manage Attributes** panel.
...
- Sort state (`sortBy`, `sortDir`) persists across saves.
