...
Recovery's buttons are the main interaction point during gameplay. The settings modal (gear icon) exposes configuration (button setup, Hit Dice pool, action assignments).
...
### Hit Dice Configuration (Settings Modal)
...
Empty module — no pre-populated buttons. The user configures from scratch via the settings modal.
...
`openRecoverySettingsModal(moduleEl, data)` (exposed as `window.openRecoverySettingsModal`) provides:
- **Manage Rest Buttons** — list with drag-to-reorder (SortableJS), edit, and delete controls per button; each action re-renders the module body immediately. **Add Rest Button** option at the bottom of the list.
  - Clicking **Edit** (or Add) opens `openRestButtonEditModal()`, a configuration modal:
...
- **Hit Dice Configuration** section (die size, total, modifier, restore policy) — applies immediately, independent of any specific button's edit modal.
- Common module settings (title, color, size) via `buildCommonSettingsSection()`.
- Rest buttons remain clickable in the module body while the settings modal is open (no disabled state — there's no accidental-trigger risk since the confirmation dialog is a separate overlay).
...
- An empty Recovery module (no buttons configured) shows a placeholder message directing the user to the Options menu.
...
- Editing the Hit Dice Configuration fields in the settings modal does not immediately re-render the module body's Hit Dice display — this is pre-existing behavior, unrelated to Phase 6.
