...
| **shared.js** | `escapeHtml(text)`, `renderMarkdown(raw)` (Marked + DOMPurify), `attachCheckboxHandlers()`, `toggleCheckboxInMarkdown()`, `inferTierPreset(systemKey)`, `getGameSystemDisplayName(systemKey)`, `buildCvSelect(options, currentValue, onChange)`, `buildStatModulePicker(selectedId, onChange, noneLabel)`, `getLinkedStatNames(moduleId)`, `initManageListSortable(container, options)` — shared destroy-and-recreate wrapper for settings-modal reorder lists |
...
| **`css/components.css`** | Shared UI components: scrollbar utility (`.cv-scroll`), drag & drop (`.cv-drag-ghost`, `.cv-drag-chosen`), toggle switches (`.cv-toggle`), icon buttons (`.icon`), base button styles, menu bar (`.menu-btn`), settings overlay form controls, custom select dropdowns, toast notifications, focus states, edit popover (`.cv-edit-popover`). |
...
- Modules span 1–8 columns; explicit position (`grid-column: colStart / span colSpan`, `grid-row: rowStart / span rowSpan`) is computed on demand, not stored — only `colSpan`, `rowSpan`, `order`, and `tabId` are persisted per module (`serializeCharacter()` in `persistence.js`)
...
- `computeLayout()` (module-core.js) derives every module's `colStart`/`rowStart` fresh on each call via a greedy scan over a per-call occupancy grid (`createOccupancyGrid()`'s `isOccupied`/`markOccupied`/`fitsAt`). Modules with an existing on-screen position are anchored there and re-placed in that same order (top-to-bottom, left-to-right); only ones whose current spot no longer fits get pushed forward to the next open slot — this is what makes a resize cascade just the modules it actually overlaps instead of re-deriving the whole tab. `applyLayout()` runs this anchored path and paints the result; it's the one used for resize, add/delete, and tab switches.
- Drag reordering uses a separate, non-anchored dense pack (`computeDenseLayout()`, driven by `previewDragLayout()`) that places every module fresh by `order` alone, live as SortableJS reorders the DOM during the drag and once more on drop. Because it ignores prior position, order-adjacent modules of matching size can end up repacked alongside whatever's being dragged rather than staying put — an accepted tradeoff in exchange for a preview that always matches the real packed result instead of the browser's native (and visually inconsistent) grid auto-placement.
