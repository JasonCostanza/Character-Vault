...
Two dropdown controls exist. Use context to choose — mixing them in the same UI causes height and style inconsistency.

**`buildCvSelect()`** — For full-width dropdowns in edit/create/detail modal form fields. Renders a styled button trigger with a floating menu. Returns `{ el, getValue(), setValue(v) }`.

```js
const { el: themeEl, getValue: getTheme } = buildCvSelect(
    [
        { value: 'fantasy', label: t('typename.theme.fantasy') },
        { value: 'scifi',   label: t('typename.theme.scifi') },
    ],
    currentValue,
    (newValue) => { working.theme = newValue; dirty = true; }
);
body.appendChild(themeEl);
```

**Native `<select>`** — For compact fields in settings grids, where height consistency with adjacent number inputs is required. Style with `.settings-select`:

```js
const select = document.createElement('select');
select.className = 'settings-select';
[{ value: 'a', label: 'Option A' }, { value: 'b', label: 'Option B' }].forEach(({ value, label }) => {
...
select.addEventListener('change', () => { working.field = select.value; dirty = true; });
```

| Context | Use |
|---|---|
| Full-width field in an edit/create/detail modal | `buildCvSelect()` from `shared.js` |
| Compact field in a settings grid alongside number inputs | Native `<select class="settings-select">` |

> **Warning:** `buildCvSelect()` attaches a `document.addEventListener('click', ...)` listener that is never removed. In a settings modal opened/closed repeatedly, this leaks one listener per open. For settings grids, native selects avoid this problem entirely and are preferred.

**Source reference:** `buildCvSelect()` in `shared.js`; the module move-to-tab selector in `scripts/sync.js` for a full-width cv-select usage example.
...
## Step 7 — Settings Modal Body Layout

Settings modals differ from edit/create modals. Edit modals stack full-width fields vertically in `.cv-modal-body`; settings modals use compact field grids. The rules in this section apply specifically to content inside `.cv-modal-body` on settings (auto-save) modals.

### Field Grid

Use a CSS Grid to group related settings fields:

```css
.typename-config-grid {
    display: grid;
    grid-template-columns: 1fr 1fr;
    gap: 6px;
}

.typename-config-field {
    display: flex;
...
Build all fields, append them to the grid, then append the grid to the body once:

```js
const grid = document.createElement('div');
grid.className = 'typename-config-grid';

// build fields, append each to grid...

body.appendChild(grid);  // grid goes in last
```

**Source reference:** `.recovery-hitdice-config-grid` in `css/sub-recovery.css`.

### Wide Fields

Fields with longer labels, longer option strings, or that would read poorly at half-width should span the full grid. Set `gridColumn` via inline style:

```js
const field = document.createElement('div');
field.className = 'typename-config-field';
field.style.gridColumn = '1 / -1';
grid.appendChild(field);  // still a grid child
```

Apply to: fields whose label truncates at half-width; the last field in an odd-count grid; dropdowns with long option text.

### Height Consistency

All controls in the same grid must share the same visual height. Mixing control types with different heights causes misalignment in the grid row.

| Consistent | Inconsistent (don't mix) |
|---|---|
| All number inputs | `buildCvSelect()` alongside `<input type="number">` |
| All `<select class="settings-select">` | cv-select trigger (taller) alongside plain inputs |
| Number inputs + styled native selects — both `padding: 4px 7px` | Any two controls with different padding or rendered height |

### All Fields Belong in the Grid

Once you have a settings grid, every related field is a grid child. Never append a settings field to the modal body with an inline margin floating beside the grid — this is always a sign the field should be inside the grid instead.

```js
// WRONG — field floats outside the grid
body.appendChild(grid);
const extra = createField();
extra.style.marginTop = '8px';   // inline margin = red flag
body.appendChild(extra);          // should be a grid child

// CORRECT
const extra = createField();
extra.style.gridColumn = '1 / -1';   // span full width
grid.appendChild(extra);              // belongs to the grid
body.appendChild(grid);               // grid goes in last
```

### Labeled Subsections

When a block of settings fields is conditionally visible or logically distinct, wrap it in a card rather than floating it beneath the grid:

```js
const section = document.createElement('div');
section.className = 'typename-config-section';
section.style.display = 'none';  // shown/hidden by another field's state
```

```css
.typename-config-section {
    padding: 10px;
...
Add a small uppercase label (`.typename-config-label`) above the card as its header. The card itself can contain its own nested grid.

**Source reference:** `.recovery-hitdice-config` in `css/sub-recovery.css`, used in `openRestButtonEditModal()` in `module-recovery.js`.

---

## Step 8 — Translation Requirements
...
function openTypenameActionModal(moduleEl, data) {
    ...
    function confirm() {
        ...
        MODULE_TYPES['typename'].renderBody(bodyEl, data);
        cancel();
    }
