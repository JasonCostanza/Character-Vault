...
Clicking a weapon card opens an **action modal** with two columns. Ctrl+Click opens the **edit modal** directly.
...
- Module body: read-only badge showing `daggerheartProficiency || 1`.
- Editing: number input in the expanded weapons settings modal (`openWeaponModuleSettings`); `min=1`, `max=6`, `step=1`; clamps live.
...
2. Register via `registerModuleType()` with `renderBody` and `overflowMenuItems`
