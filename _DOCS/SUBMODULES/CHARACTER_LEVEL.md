...
## UI/UX
Each Level module can optionally have a **class name** (e.g., "Fighter", "Ranger") that displays above the level number in place of the generic "LEVEL" label. This is especially useful for multiclass characters who use multiple Level modules — one per class. The class name is set via Level Settings and renders uppercase to match the label styling.
...
- Ctrl+Click the level number to manually set the current level.
- Access the module settings via the overflow menu to customize the leveling system and progress bar appearance.
...
- Set an optional **Class Name** (e.g., "Fighter", "Ranger") that displays above the level number instead of the generic "LEVEL" label.
