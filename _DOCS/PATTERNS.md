...
### Ability Linking
- **`window.refreshLinkedAbilitiesChainIcons(statModuleId)`** — Re-renders the chain-link indicator on Abilities modules linked to a specific Stat module. Called by Module Core when a Stat module is renamed.
...
Stats can be "linked" to Abilities, Saving Throws, and Weapons so those modules pull a numeric modifier from the linked Stat. Proficiency, however, is never pulled from the stat link — it is always owned per-item.

### How It Works

1. **Abilities/Saving Throws/Weapons module** stores `data.content.linkedStatModuleId` — the module ID of a Stat module to link against.
2. **Each item** (ability/save/weapon) stores its own key naming which stat to pull from (`ability.linkedStat`, `save.linkedStatName`, `weapon.abilityMod`/`weapon.governingTrait`).
3. When rendering or rolling, the module calls `window.getAbilityModifierFrom(statName, linkedStatModuleId)` (defined in `module-stat.js`) to fetch the linked stat's numeric `.modifier`. This is the *only* thing the stat link contributes.
4. Proficiency is stored directly on the item itself — `ability.proficiency`/`ability.proficiencyRank`, `save.proficiencyTier`, `weapon.proficient`/`weapon.proficiencyRank` — and is never read from the linked Stat module. Renaming a linked stat does not change an item's proficiency state.

### Proficiency Bonus Resolution

- **D&D 5e / Custom**: `window.getProficiencyBonus()` (in `module-stat.js`) derives the bonus from total character level (`Math.floor((level-1)/4)+2`) when a Level module exists, falling back to the `isProficiencyStat`-flagged Stat's `.value`, then to `2`. Each item adds this bonus to its roll/display only if its own boolean proficiency flag (or `proficiencyTier === 'Proficient'` for saves) is set.
- **PF2e**: `window.computePf2eProficiencyBonus(rank)` (in `shared.js`) maps the item's own rank (`'untrained'|'trained'|'expert'|'master'|'legendary'`) to a bonus and adds character level. Every item computes its own bonus from its own rank — there is no rank inheritance from a linked Stat.

### Reactive Updates

- **`cv:level-changed`** (dispatched by the Level module) — the Stat module recomputes and re-renders the `isProficiencyStat` stat; Abilities, Saving Throws, and Weapons modules each independently re-render all their instances, since the proficiency bonus (D&D 5e level-based, or PF2e level-in-formula) may have changed.
- **`cv:stat-values-changed`** (dispatched by the Stat module when a stat's value/modifier is edited) — Abilities, Saving Throws, Weapons, Spells, and Text modules re-render any instances linked to the changed Stat module, refreshing the numeric modifier pulled via `getAbilityModifierFrom`.
