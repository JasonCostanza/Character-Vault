...
| `renderBody(bodyEl, data)` | Renders a `.spacer-controls` div with drag handle, label, and delete button (hover-revealed). |
...
- `.spacer-controls` renders inline in the `.module-body` and is **hover-revealed** (opacity transitions from 0 → 1 on hover):
...
## Visibility
- When not hovered, `.spacer-controls` has opacity 0 (controls hidden but still interactable).
- CSS rule `.module[data-type="spacer"]` uses:
...
- The resize handle remains visible on hover alongside the spacer controls.
