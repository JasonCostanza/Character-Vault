...
All spell data is edited inline — no modals:
- **Category header**: drag handle, collapse button, name input, slot level select, delete button
- **Table header**: drag spacer, chevron spacer, Name (sortable), pinned attribute columns (sortable), delete spacer
- **Spell row**: drag handle, chevron, name input, attribute value inputs, delete button
- **Drawer**: textarea for the spell description (commits on `input`)
- **Add Spell** button below each category table
- **Add Category** button at the bottom of the container

### SortableJS

- Spell rows within a category: `handle: '.spells-row-handle'`, `draggable: '.spells-row'` (drawer rows excluded). After `onEnd`, re-render the tbody to re-associate drawers.
- Category blocks: `handle: '.spells-cat-drag-handle'`, `draggable: '.spells-category-block'`. After `onEnd`, update `content.categories` order and save.
- Both sortables are only initialized when `sortBy === null` (manual order mode).
