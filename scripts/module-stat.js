...
const STAT_TEMPLATES = {
    ...
    daggerheart: [
        { name: 'Agility' }, { name: 'Strength' }, { name: 'Finesse' },
        { name: 'Instinct' }, { name: 'Presence' }, { name: 'Knowledge' }
    ],
    coc: [
        { name: 'STR' }, { name: 'CON' }, { name: 'SIZ' },
        { name: 'DEX' }, { name: 'APP' }, { name: 'INT' },
        { name: 'POW' }, { name: 'EDU' }, { name: 'LCK' }
    ],
    vtm: [
    ...
    cpred: [
        { name: 'INT' }, { name: 'REF' }, { name: 'DEX' },
        { name: 'TECH' }, { name: 'COOL' }, { name: 'WILL' },
        { name: 'LUCK' }, { name: 'MOVE' }, { name: 'BODY' },
        { name: 'EMP' }
    ],
    mothership: [
        { name: 'Strength' }, { name: 'Speed' },
        { name: 'Intellect' }, { name: 'Combat' }
    ],
    sr6: [
        { name: 'Body' }, { name: 'Agility' }, { name: 'Reaction' },
        { name: 'Strength' }, { name: 'Willpower' }, { name: 'Logic' },
        { name: 'Intuition' }, { name: 'Charisma' }, { name: 'Edge' }
    ]
};
...
function applyStatTemplate(templateKey) {
    ...
    return template.map(t => ({
        name: t.name, value: 0, modifier: 0, proficient: false, rollable: true
    }));
}
...
function renderStatBlock(stat, index, data, isPlayMode) {
    ...
    if (isPlayMode && stat.rollable) {
        block.addEventListener('click', (e) => {
            if (e.ctrlKey) { enterQuickEdit(block, stat, data); return; }
            rollStatCheck(stat);
        });
    }
...
function renderStatBlockEdit(stat, index, data) {
    ...
    nameInput.addEventListener('input', () => { stat.name = nameInput.value; scheduleSave(); });
    valInput.addEventListener('input', () => { stat.value = parseInt(valInput.value, 10) || 0; scheduleSave(); });
    modInput.addEventListener('input', () => { stat.modifier = parseInt(modInput.value, 10) || 0; scheduleSave(); });
    profCb.addEventListener('change', () => { stat.proficient = profCb.checked; scheduleSave(); });
    ...
    block.addEventListener('click', (e) => {
        ...
        if (target.tagName === 'INPUT' || target.tagName === 'BUTTON' || target.closest('button') || target.closest('label')) return;
        const container = block.closest('.stat-container');
        const moduleEl = container.closest('.module');
        if (!moduleEl) return;
        ...
        if (moduleEl._selectedStatIndex !== null) {
            const selectedBlock = container.querySelector(`.stat-block-edit[data-index="${moduleEl._selectedStatIndex}"]`);
