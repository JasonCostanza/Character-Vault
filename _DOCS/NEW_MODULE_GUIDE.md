...
| `scripts/module-core.js` | Creation defaults in wizard handler | Yes |
...
(function () {
    ...
    registerModuleType('typename', {
        ...
        renderBody(bodyEl, data) {
            ...
            // Build your UI here — display + interaction in a single render path
        ...
        overflowMenuItems(moduleEl, data) {
            return [
                {
                    label: t('typename.settings'),
                    icon: '<svg class="icon" ...>...</svg>',
                    onClick: () => openTypenameSettings(moduleEl, data),
                },
            ];
        },
    });

    // ── Window Exports (pure functions for vitest) ──
    // DOM/render/event handlers stay private inside the IIFE.
    window.someTestableFunction = someTestableFunction;
})();
```
...
|---|---|---|
| `renderBody(bodyEl, data)` | Populate `.module-body` with your UI | Module creation, load, data change |

### Optional Methods

| Method | Purpose | When to Include |
|---|---|---|
| `overflowMenuItems(moduleEl, data)` | Return `[{ label, icon, onClick }]` entries for the overflow (kebab) menu | If your module has actions (settings, add item, etc.) beyond Rename/Delete |

### Pattern Notes

- **Unified Render** — There is no Play/Edit mode split. `renderBody()` produces a single UI where display values support Ctrl+Click quick editing (via `window.openEditPopover`) and structural editing (add/delete/reorder) happens through the settings modal. The module body is always the same render path.
...
**Current alphabetical order:** See `_DOCS/ARCHITECTURE.md` § MODULE_TYPES for the full list
...
### B. Overflow Menu Items — (if your type has actions)

Module-specific overflow menu entries are defined via the `overflowMenuItems()` hook in your `registerModuleType()` call (see Step 2). The core builds the overflow menu as: **Rename** → your entries → **Delete**. No changes to `module-core.js` are needed.

### C. Theme/Resize Exclusions — (if your type shouldn't have these)
...
### B. XS Breakpoint Adjustments — (if your module needs compact styling)
...
- [ ] Create `scripts/module-TYPENAME.js` with IIFE + registerModuleType()
  - [ ] Implement `renderBody(bodyEl, data)` — single unified render path
  - [ ] Implement `overflowMenuItems(moduleEl, data)` — return type-specific menu entries
  - [ ] Expose pure functions on `window` for vitest (rule 19)
