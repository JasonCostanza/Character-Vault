...
## Interactions
The module displays companions in a table with editable attribute values. Each entry is a text input field that allows direct editing. These fields support simple arithmetic operations like addition and subtraction. Key interactions:
- Managing Companion HP, AC, Speed
- Tracking if a Companion is active or inactive
- Dragging attributes to reorder them in the table
- Module-level operations (resize, move, rename, theme) via the standard toolbar/overflow menu
