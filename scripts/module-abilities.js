...
function renderAbilityRow(ability, index, data) {
    ...
    const abbrev = ability.linkedStat
        ? ability.linkedStat.substring(0, 3).toUpperCase()
        : (ability.abbrev || '');
...
function renderAbilityRowEdit(ability, index, data) {
    ...
    nameInput.addEventListener('input', () => { ability.name = nameInput.value; scheduleSave(); });
    modInput.addEventListener('input', () => { ability.modifier = parseInt(modInput.value, 10) || 0; scheduleSave(); });
    if (abbrevInput) {
...
function initAbilitySortable(container, data) {
    container._sortable = new Sortable(container, {
        ...
        onEnd() {
            ...
            const reordered = items.map(el => data.content.abilities[parseInt(el.dataset.index, 10)]).filter(Boolean);
...
function openAbilitySettings(moduleEl, data) {
    ...
    closeBtnEl.innerHTML = '<svg class="icon" xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>';
