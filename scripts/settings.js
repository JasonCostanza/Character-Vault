...
(function () {
    ...
    // ── POC: Export/Import Test ──
    const pocSection = document.createElement('div');
    pocSection.className = 'settings-section';
    pocSection.style.cssText = 'border-top: 1px dashed var(--cv-border); padding-top: 10px; margin-top: 4px;';

    const pocLabel = document.createElement('label');
    pocLabel.className = 'settings-label';
    pocLabel.textContent = 'POC: File I/O Test';
    pocSection.appendChild(pocLabel);

    const pocBtnRow = document.createElement('div');
    pocBtnRow.className = 'settings-btn-row';

    const btnPocExport = document.createElement('button');
    btnPocExport.className = 'settings-action-btn btn-primary';
    btnPocExport.textContent = 'Test Export';
    btnPocExport.addEventListener('click', function () {
        try {
            ...
            a.href = url;
            a.download = 'cv-test-export.json';
            document.body.appendChild(a);
            a.click();
            document.body.removeChild(a);
            URL.revokeObjectURL(url);
            console.log('[CV] POC export: download triggered');
            showToast('Export test: download triggered', 'success');
        } catch (e) {
            console.error('[CV] POC export failed:', e);
            showToast('Export test: failed — see console', 'error');
        }
    });

    const btnPocImport = document.createElement('button');
    btnPocImport.className = 'settings-action-btn btn-primary';
    btnPocImport.textContent = 'Test Import';
    btnPocImport.addEventListener('click', function () {
        ...
        input.addEventListener('change', function () {
            ...
            reader.onload = function (e) {
                try {
                    ...
                    console.log(`[CV] POC import: parsed OK, ${keyCount} keys`, parsed);
                    showToast(`Import test: read OK (${keyCount} keys)`, 'success');
                } catch (err) {
                    console.error('[CV] POC import: JSON parse failed:', err);
                    showToast('Import test: JSON parse failed — see console', 'error');
                }
                document.body.removeChild(input);
            };
            reader.onerror = function () {
                console.error('[CV] POC import: FileReader error');
                showToast('Import test: file read failed — see console', 'error');
                document.body.removeChild(input);
            };
    ...
    pocBtnRow.appendChild(btnPocExport);
    pocBtnRow.appendChild(btnPocImport);
    pocSection.appendChild(pocBtnRow);
    document.querySelector('#settings-overlay .settings-body').appendChild(pocSection);
    // ── End POC: Export/Import Test ──
